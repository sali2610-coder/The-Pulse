...
// Advanced manual-OCR entry. Behind a collapsed disclosure so it
// doesn't clutter Settings. Paste the receipt text, get back the
// fields the parser extracted. The user copies the values into the
// regular new-expense form. NO automatic addExpense — the foundation
// is deliberately read-only until a paid OCR provider is wired.
...
export function ReceiptScanCard() {
  ...
  async function runScan() {
    ...
    setScanning(true);
    try {
      const provider = pickReadyOcrProvider();
      ...
      {open ? (
        <div className="mt-3 flex flex-col gap-2">
          <p className="text-[11px] text-muted-foreground/85">
            הדבק את הטקסט מהקבלה. אנחנו ננסה לחלץ סכום, בית עסק ותאריך.
            הוספה ידנית בלבד — לא נכתב חיוב אוטומטית.
          </p>
          <textarea
            value={raw}
