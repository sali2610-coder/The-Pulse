...
export async function POST(req: Request): Promise<NextResponse> {
  // Phase 389 — read the server-only env var. Never accept it from
  // the client, never echo it back. If missing, return a calm
  // user-facing Hebrew message instead of the technical env name.
    ...
    return NextResponse.json(
      {
        ...
        message:
          "סריקת קבלה עדיין לא פעילה. חסר מפתח מערכת בצד השרת.",
      },
      { status: 503 },
    );
  }
  ...
  // Phase 389 — 30s timeout. The user is staring at a spinner; do
  // not let the request hang silently.
    ...
    return NextResponse.json(
      {
        ...
        message: aborted
          ? "הסריקה ארכה יותר מדי. נסה שוב."
          : "לא ניתן להתחבר למנוע סריקה כרגע.",
      },
      { status: 504 },
    );
  }
